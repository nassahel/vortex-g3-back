...
export class CartService {
    ...
    async upsertCartItem(userId: string, upsertCartItemDto: UpsertCartItemDto): Promise<{ message: string }> {
        ...
        const existingItem = await this.prisma.orderItem.findFirst({    //Busca el item en el carrito
        ...
        if (existingItem) {
            if (quantity === 0) {
                ...
                await this.prisma.orderItem.update({    //Actualiza la cantidad del item
            ...
            await this.prisma.orderItem.create({    //Crea un nuevo item en el carrito
                data: {
                    orderId: cart.id,
                    productId,
                    quantity,
    ...
    async removeItemFromCart(userId: string, productId: string): Promise<{ message: string }>{
        ...
        const item = await this.prisma.orderItem.findFirst({    //Busca el item en el carrito
            where: { orderId: cart.id, productId },
        });
    ...
    async checkoutCart(userId: string, checkoutCartDto: CheckoutCartDto): Promise<{ message: string, cart: object }>{
        ...
        const completedCart = await this.prisma.order.update({  //Actualiza el carrito a 'COMPLETED'
            where: {id: cart.id},
    ...
    async cancelCart(userId: string): Promise<{ message: string }>{
        ...
        const cancelCart = await this.prisma.order.update({ //Actualiza el carrito a 'CANCELED'
            where: {id: cart.id},
