...
export class AwsService {
  ...
  async uploadImage(
    ...
    const fileExtensionsByMimetype: { [key: string]: string } = {
      //mapea el mimetype del archivo a su extension correspondiente. es para asegurarse que se esta subiendo en la extension correcta a su tipo
      'image/jpeg': 'jpg',
      'image/png': 'png',
      'image/gif': 'gif',
      'image/tiff': 'tiff',
    ...
    try {
      ...
      await this.s3Client.send(
        //.send maneja la comunicacion con aws para cargar el archivo
        new PutObjectCommand({
          //es una solicitud para poder subir un archivo a s3
    ...
    try {
      await this.s3Client.send(
        new DeleteObjectCommand({
          //Borra el objeto en el bucket de s3
          Bucket: awsConfig.s3.bucket,
          Key: key,
        }),
      );
